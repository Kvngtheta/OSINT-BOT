...
def summarize_actionable(article_text: str, source_url: str, client: OpenAI) -> str:
	...
	" Ensure you replies are 1024 characters or less "
    )
    user = (
        f"Source: {source_url}\n\n"
...
def google_news_search_rss(query: str, hl: str = "en-US", gl: str = "US", ceid: str = "US:en") -> str:
    """
    Build a Google News RSS search URL.
    Supports operators like quotes, OR, site:, and recency hints like when:7d.
    Example: google_news_search_rss("Ivanti CVE when:7d")
    """
            ...
            for msg in fetched:
                try:
                    ...
                    for url in urls[:3]:  # limit per message
                        ...
                        # Summarize
                        summary_json = (summarize_actionable, text, url, client_openai)
...
async def intel_help(interaction: discord.Interaction):
    ...
    embed.add_field(name="/summarize_recent count:<1-100>", value="Summarize the last N messages with links from the news channel.", inline=False)
...
async def triage(interaction: discord.Interaction, url: str):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        summary_json = clean_json_string(summary_json)
...
async def intel(interaction: discord.Interaction, query: str):
    ...
    indicators = [q]
    async with aiohttp.ClientSession() as session:
        # 1) Your static feeds (CISA, NCSC, MSRC, etc.)
        static_hits = await cross_reference(session, indicators)

        # 2) Google News dynamic RSS search for the same query
        try:
            g_url = google_news_search_rss(q)  # supports: quotes, OR, site:, when:7d
            g_hits = await fetch_feed_entries(session, g_url, limit=12)
        except Exception:
            g_hits = []
        ...
        seen_links = set()
        xrefs = []
        for src in (g_hits or []) + (static_hits or []):
            link = (src.get("link") or "").strip()
            if link and link not in seen_links:
                seen_links.add(link)
                xrefs.append(src)

    # Format a quick blurb from combined results
    items = []
    for e in xrefs[:8]:
    ...
    text_blurb = "\n\n".join(items) or "No matches found."

    # Optional: summarize with OpenAI
    if client_openai and text_blurb != "No matches found.":
        system = ("You are a cyber threat intel analyst. Summarize the items into a single "
...
async def summarize_recent(interaction: discord.Interaction, count: app_commands.Range[int, 1, 100] = 5):
    await interaction.response.defer(thinking=True)
